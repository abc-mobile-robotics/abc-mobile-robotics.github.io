...
flowchart LR
    subgraph WOLF["Wolf laptop independent ROS 2 graph"]
        direction TB
        WCAM["OAK-D camera /oakd/rgb/preview/image_raw"]
        WLIDAR["2-D LiDAR /scan"]
        WAMCL["AMCL pose /amcl_pose"]
        WYOLO["yolo_detector best.pt"]
        WGEO["geofence_node wolf polygon"]
        WFSM["WolfFSM PATROL CHASE RETURN_TURN STOP_CAUGHT"]
        WKF["RabbitPixelKF and PID chase controller"]
    ...
    subgraph RABBIT["Rabbit laptop independent ROS 2 graph"]
        direction TB
        RCAM["OAK-D camera /oakd/rgb/preview/image_raw"]
        RLIDAR["2-D LiDAR /scan"]
        RODOM["Odometry /rabbit/odom"]
        RYOLO["yolo_detector best.pt"]
        RGEO["geofence_node rabbit polygon"]
        RFSM["RabbitFSM WANDER RETURN_HOME REST DEAD"]
        RHOME["Home pose locked from first odom"]
    ...
    WCAM -. sees rabbit sign .-> RFSM
    RCAM -. sees wolf sign .-> WFSM
```

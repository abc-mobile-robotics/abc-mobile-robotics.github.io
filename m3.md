...
$$\omega = -\,\text{clamp}\!\left(K_p e + K_i \int e\,dt + K_d \dot{e}_{filt},\ -\omega_{max},\ \omega_{max}\right)$$
...
$$p_x \leftarrow p_x + v_x\,\Delta t - \omega_{cmd}\,\Delta t\,\rho_{px/rad}$$
...
$$\theta_{tgt} = \operatorname{atan2}(y_h - y, x_h - x), \quad \omega = \text{clamp}(K_\theta\,\text{wrap}(\theta_{tgt}-\theta),\ -\omega_{max},\ \omega_{max})$$
Forward speed is gated: while $|\theta_{err}| > \texttt{return\_yaw\_drive\_threshold}$ the rabbit only rotates; once aligned it drives forward, scaled by $1 - 0.45\,|\omega|/\omega_{max}$ so heavy turns slow it down.

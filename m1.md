...
flowchart TD
  ...
  subgraph RFA ["RABBIT FOOD FINDING AREA"]
    A([start]) --> B[ROBOT A\nRabbit]
    B --> C[roaming around randomly]
    C --> D{finding food?}
    D -- NO --> C
    D -- YES --> E[Head to the carrot]
    E --> F[eat the carrot]
    F --> G{SPOT A WOLF?}
    G -- NO --> C
    G -- YES --> H[RUN AWAY]
    H --> I{WOLF CATCHES THE RABBIT\nbefore it ESCAPED?}
    I -- NO\n30 seconds cooldown --> C
    I -- YES --> J([END])
  end

  %% === ROBOT B (WOLF) - Hunting Ground ===
  subgraph HG ["HUNTING GROUND"]
    K([START]) --> L[ROBOT B\nWolf]
    L --> M[SPIN around\nINSIDE HUNTING GROUND]
    M --> N{Look for Rabbit?}
    N -- no --> M
    N -- yes --> O[chase the rabbit]
    O --> P{WOLF catches\nthe rabbit?}
    P -- NO\n30 seconds cooldown --> M
    P -- YES --> Q([END])
  end
```
**Robot A — Rabbit (Food-Finding Area):**
...
<iframe
  ...
  src="https://viewer.diagrams.net/?url=https://github.com/abc-mobile-robotics/abc-mobile-robotics.github.io/blob/main/assets/milestone1.drawio">
</iframe>
